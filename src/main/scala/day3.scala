...
object day3 {
  ...
  val input = """.......358..........31.....339.....669.............598......328.....575......................447..650..............964...........692........
                ...
                |.............963......................542........734.....901...914..........843.............523..........818..................691.....833...""".stripMargin

  def matrixSize(input: String): (Int, Int) = {
    val nbRow = input.linesIterator.size
    val nbCol = input.linesIterator.map(_.length).toList.max
    //input.linesIterator.map(_.filter(_.isDigit)).toList
    //(input.linesIterator.map(_.size),input.linesIterator.size)
    (nbRow, nbCol)
  }

  val mSize: (Int, Int) = matrixSize(exemple)
  val a: Array[Array[String]] = Array.ofDim[String](mSize._1,mSize._2)

  def parse(input: String): Array[Array[Char]] = {
    input.split("\n").map(_.toArray)
  }

  val matrice: Array[Array[Char]] = parse(exemple)




  def fillList(matrice:  Array[Array[Char]], mSize: (Int,Int)): ( List[Symbol],Seq[PartNumber] ) = {
    var listSymbol: List[Symbol] = List.empty[Symbol]
    var listPartnumber: Seq[PartNumber] = Seq.empty[PartNumber]
    for (i <- 0 until mSize._1) {
      for (j <- 0 until mSize._2) {
        matrice(i)(j) match {
          case x if (x.isDigit) =>
            if (j > 0 && matrice(i)(j - 1).isDigit) {
              print(s"$x already managed")
            } else {
              var k = 1
              var nextNumber = Try(matrice(i)(j + k)) // todo try and length 1
              var number = s"$x"
              while (nextNumber.isSuccess && nextNumber.map(_.isDigit).get) {
                number = number + nextNumber.get
                k = k + 1
                nextNumber = Try(matrice(i)(j + k))
              }
              listPartnumber = listPartnumber :+ PartNumber(number.toInt, Coord(i, j), Coord(i, j + (k - 1)))
            }

          case '.' => print(s"not a symbol nor digit : ${Coord(i, j)}")
          case x: Char => listSymbol = listSymbol :+ Symbol(x.toString, Coord(i, j))

        }
      }
    }

    (listSymbol, listPartnumber)
  }

  def findIntersect(number: PartNumber, symbol: Symbol): Boolean = {
    val symbolCoord: Coord = symbol.pos
    val coordNumber = List(number.start, number.end)
    val validCoord = List(
      Coord(symbolCoord.x - 1, symbolCoord.y - 1), Coord(symbolCoord.x - 1, symbolCoord.y), Coord(symbolCoord.x - 1, symbolCoord.y + 1),
      Coord(symbolCoord.x, symbolCoord.y - 1), symbolCoord, Coord(symbolCoord.x, symbolCoord.y + 1),
      Coord(symbolCoord.x + 1, symbolCoord.y - 1), Coord(symbolCoord.x + 1, symbolCoord.y), Coord(symbolCoord.x + 1, symbolCoord.y + 1)
    )
    val res: List[Coord] = coordNumber.intersect(validCoord)
    res.nonEmpty


  }

  def isValidPartNumber(number: PartNumber, symbols: List[Symbol]): Boolean = {
    var res = false
    for (symbol <- symbols) {
      if (findIntersect(number, symbol)) {
        res = true
        return res
      } else {
        res = false
  ...
  def findValueGear(numbers: Seq[PartNumber], symbol: Symbol): Int = {
    var res: Seq[Int] = Seq.empty[Int]
    for (number <- numbers) {
      if (findIntersect(number, symbol)) {
        res = res :+ number.value
      }
    }

    val valueGear = if (res.length < 2) 0 else res.product
    valueGear
  }

  val resFill = fillList(matrice, day3.mSize)
  val listSymbol = resFill._1
  val listPartnumber = resFill._2

  val validNumber: Seq[(PartNumber, Boolean)] =  listPartnumber.map(x => (x, isValidPartNumber(x, listSymbol))).filter(x => x._2)

  def findResP1(input3 : String): Int = {
    val mSize: (Int, Int) = matrixSize(input3)
    val matrice: Array[Array[Char]] = parse(input3)

    val resFill = fillList(matrice, mSize)
    ...
    validNumber.map(x => x._1.value).sum

  }

  val resultP1 = findResP1(input)


  def findResP2(input3: String): Int = {
    val mSize: (Int, Int) = matrixSize(input3)
    val matrice: Array[Array[Char]] = parse(input3)

    val resFill = fillList(matrice, mSize)

    val listSymbol = resFill._1.filter(_.sym == "*")
    val listPartnumber = resFill._2

    val valuesGear: Seq[(Symbol, Int)] = listSymbol.map(x => (x, findValueGear(listPartnumber, x)))

    valuesGear.map(x => x._2).sum
